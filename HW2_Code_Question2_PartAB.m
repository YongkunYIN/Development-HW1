...
% set Corr_S = 1 if the seasonal parts of consumption and labor are
% positively correlated;
% Set Corr_S = -1 if they are negatively correlated.
...
kappa = -2*0.66/(28.5*30/7)^2;   % see the solution document for calibration

sigma_e1 = 0.2^(1/2);    % standard deviation of epsilon
sigma_u = 0.2^(1/2);     % standard deviation of u.
...
if Corr_S==1
    rho_S = 1;                  % correlation coefficient between consumption and labor supply.
    G_labor = G;
else
    rho_S = -1;
    G_labor = [1.070, 1.140, 1.280;...  % if correlation coeff. is -1, make labor supply follow opposite pattern.
    1.076, 1.151, 1.303;...
    0.845, 0.691, 0.381;...
    0.932, 0.863, 0.727;...
    0.961, 0.921, 0.843;...
...
Sigma_m = [0.043, 0.085, 0.171;...  % monthly specific standard deviation.
0.034, 0.068, 0.137;...
0.145, 0.290, 0.580;...
0.142, 0.283, 0.567;...
0.137, 0.273, 0.546;...
...
c = @(z,g,e2,e1)(z * g * e2 * e1);     % consumption
h = @(z,g,e2,e1)(z * g * e2 * e1);     % labor supply
...
E1_consum = normrnd(0, sigma_e1,N,Y*12);
...
E1_labor = normrnd(0, sigma_e1,N,Y*12);
...
C_o = zeros(N,Y*12,9);    % consumption
H_o = zeros(N,Y*12,9);    % labor supply
W_o = zeros(N,9);         % welfare/ life time utility
for j1=1:1:3
    for j2 = 1:1:3
        for i=1:1:N
            for y=1:1:Y
...
C_noSC = zeros(N,Y*12,9);     % consumption
H_noSC = zeros(N,Y*12,9);     % labor supply
W_noSC = zeros(N,9);          % welfare
for j1=1:1:3
    for j2=1:1:3
        for i=1:1:N
            for y=1:1:Y
...
% supply.
C_noSCH = zeros(N,Y*12,9);     % consumption
H_noSCH = zeros(N,Y*12,9);     % labor supply
W_noSCH = zeros(N,9);          % welfare
for j1=1:1:3
    for j2=1:1:3
        for i=1:1:N
            for y=1:1:Y
